...
本リポジトリは、WordPress のデータを a-blog cms にインポートするためのプラグイン（拡張アプリ） です。
...
> **原則：バグ修正・軽微な不具合対応ではステアリングを作成しない**
>
> 既存挙動を仕様通りに戻すだけのバグ修正、ピンポイントなリファクタ、ドキュメント微修正など「設計判断を伴わない修正タスク」では、`.steering/` 配下のディレクトリやドキュメントを作成せず、即座に修正に着手してよい。
> ステアリング作成が必要なのは、**新規機能追加・既存機能の仕様変更・アーキテクチャ変更など、要件・設計・タスク分解の合意形成が必要なケースに限る**。判断に迷う場合のみ、修正に入る前にユーザーへ確認する。
